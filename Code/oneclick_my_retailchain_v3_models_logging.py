...
def answer_sales_ceo_kpi(q: str):
    ...
    if is_compare:
        ...
        # Apply same filters to both months
        if state:
...
def retrieve_context(query: str, k: int = 12, mode: str = "all") -> str:
    ...
    # ambil lebih awal supaya boleh filter
    k0 = min(max(k * 5, 40), index.ntotal)
...
HR_KEYWORDS = [
    ...
    "attrition", "resign", "turnover", "overtime", "ot", "salary", "gaji", "income", "monthlyincome"
]
...
def rag_query_ui(user_input: str, model_name: str, has_image: bool = False) -> str:
    ...
    try:
        ...
        if len(user_input) < 3:
            ...
            safe_log_interaction(model_name, "validation", user_input, ans, latency)
            return ans

        intent = detect_intent(user_input, has_image=has_image)

	# ✅ DEBUG (sementara) — confirm callback jalan + route apa
        print("DEBUG intent=", intent, "model=", model_name, "len=", len(user_input), "has_image=", has_image)

        if intent == "visual":
            route = "visual"
            ans = "📷 **Source: OCR + RAG + LLM**\n" + generate_answer_with_model(model_name, user_input)
            latency = int((time.perf_counter() - start) * 1000)
            safe_log_interaction(model_name, route, user_input, ans, latency)
            return ans

        if intent == "hr_kpi":
            route = "hr_kpi"
            hr_ans = answer_hr(user_input)
            ans = hr_ans if hr_ans is not None else "HR info not available in provided data."
            latency = int((time.perf_counter() - start) * 1000)
            safe_log_interaction("N/A", route, user_input, ans, latency)
            return ans
        ...
        route = "rag_docs"
        ans = "📄 **Source: RAG + LLM**\n" + generate_answer_with_model(model_name, user_input, mode="docs")
        latency = int((time.perf_counter() - start) * 1000)
        safe_log_interaction(model_name, route, user_input, ans, latency)
        return ans
...
def multimodal_query(text_input, image_input, model_name):
    ...
    has_image = image_input is not None  # ✅ ADD THIS LINE

    if has_image:
        cap = caption_image(image_input)
        print("🖼️ OCR:", cap[:300])  # optional: avoid huge log
        if query:
            query = f"{query}\n\n{cap}"
        else:
    ...
    return rag_query_ui(query, model_name, has_image=has_image)
