...
def rag_query_ui(user_input: str, model_name: str, has_image: bool = False):
    ...
    try:
        ...
        if len(user_input) < 3:
            full_ans = "Sila tanya soalan yang lebih jelas."
            latency = int((time.perf_counter() - start) * 1000)
            safe_log_interaction(model_name, "validation", user_input, full_ans, latency)
            yield full_ans
            return
        ...
        # 1) Visual path (OCR text already appended in multimodal_query)
        if intent == "visual":
            route = "visual"
            prefix = "📷 **Source: OCR + RAG + LLM**\n"
            yield prefix  # cepat bagi user nampak output terus

            out = ""
            for partial in generate_answer_with_model_stream(model_name, user_input, mode="all"):
                out = partial
                yield prefix + out

            full_ans = (prefix + out).strip()
            latency = int((time.perf_counter() - start) * 1000)
            safe_log_interaction(model_name, route, user_input, full_ans, latency)
            return
        ...
        if intent == "hr_kpi":
            ...
            if hr_ans is None:
                ...
                out = ""
                for partial in generate_answer_with_model_stream(model_name, user_input, mode="docs"):
                    out = partial
                    yield prefix + out
            ...
            full_ans = hr_ans
            latency = int((time.perf_counter() - start) * 1000)
            safe_log_interaction("N/A", route, user_input, full_ans, latency)
            yield full_ans
            return
...
# =========================
# 9) Multimodal wrapper (image + text) — STREAMING
# =========================
def multimodal_query(text_input, image_input, model_name):
    ...
    # ✅ BETUL: guna router + logging + streaming
    yield from rag_query_ui(query, model_name, has_image=has_image)
...
if __name__ == "__main__":
    ...
    ).queue().launch(inbrowser=True, debug=True, show_error=True)
