...
# =========================
# 10) Launch UI — UPDATED (minimal, less jitter)
# =========================
import gradio as gr
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="MY Retail Chain VL-RAG Assistant (Sales + HR + Docs) — Compare LLM") as demo:
        # ✅ Minimal CSS: fix answer height so UI tak tolak turun/naik
        gr.HTML("""
        <style>
          #status_box { min-height: 28px; }
          #answer_box {
            height: 260px;
            overflow: auto;
        ...
        gr.Markdown("## MY Retail Chain VL-RAG Assistant (Sales + HR + Docs) — Compare LLM")
        ...
        # ✅ Status fixed at top (timer/processing)
        status_md = gr.Markdown("", elem_id="status_box")

        # ✅ Answer output below (fixed height + scroll)
        answer_md = gr.Markdown("", elem_id="answer_box")

        with gr.Row():
            txt = gr.Textbox(lines=2, label="Soalan (optional)")
            img = gr.Image(type="filepath", label="Upload table/chart image (optional)")
            model = gr.Dropdown(choices=models, value=default_model, label="Choose LLM model (RAG only)")
