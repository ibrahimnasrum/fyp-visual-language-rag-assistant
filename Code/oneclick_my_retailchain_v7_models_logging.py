...
def rag_query_ui(user_input: str, model_name: str, has_image: bool = False):
    ...
    def status_line(text: str) -> str:
        return f"_⏳ {text} ({elapsed_s():.1f}s)_"
...
def multimodal_query(text_input, image_input, model_name):
    ...
    yield (f"_⏳ Processing... {elapsed():.1f}s_  \n_Model: `{model_name}`_", "")
...
# =========================
# 10) Launch UI — POLISHED + COMPATIBLE (No unsafe_allow_html)
# =========================
import gradio as gr
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="MY Retail Chain Assistant") as demo:
        # ✅ Polished CSS (still compatible with older Gradio)
        gr.HTML("""
        <style>
          body {
        ...
        with gr.Column(elem_classes=["container"]):
            ...
            # ✅ Hero header (HTML only for layout, not Markdown unsafe HTML)
            gr.HTML("""
            <div class="hero">
              <div class="title">🇲🇾 MY Retail Chain Assistant</div>
              <div class="subtitle">Enterprise Analytics & Policy Intelligence (FAISS + OCR + RAG + LLM)</div>
              <div class="pillrow">
                <div class="pill">Sales/HR KPI: Deterministic</div>
                <div class="pill">Policy/SOP: Docs RAG</div>
                <div class="pill">Image: OCR + RAG</div>
                <div class="pill">Streaming + Timer</div>
              </div>
            </div>
            """)
            ...
            with gr.Row():
                ...
                with gr.Column(scale=5):
                    ...
                    gr.HTML("""
                    <div class="hint">
                      <b>Tip demo:</b> Pilih model (kanan) untuk RAG, kemudian cuba 1 soalan KPI + 1 soalan policy + 1 upload image.
                    </div>
                    """)
