...
def extract_month_from_query(q: str, previous_context: dict = None):
    """Enhanced month extraction with natural language support"""
...
def detect_intent(text: str, has_image: bool, conversation_history: list = None) -> str:
    ...
    Priority:
      ...
      5) Check conversation history for context clues
      6) Default => rag_docs
    ...
    # NEW: Check conversation history for context clues (with word-boundary matching)
    if conversation_history and len(conversation_history) > 0:
        # Get last user query
        last_user_msg = None
