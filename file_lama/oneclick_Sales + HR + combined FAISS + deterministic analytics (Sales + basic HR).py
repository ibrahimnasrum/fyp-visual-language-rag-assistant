...
def is_hr_department_count_question(query: str) -> bool:
    ...
    if "how many" not in q:
        return False
    # department keyword or specific department name
    if "department" in q:
        return True
...
def answer_hr_department_count(query: str) -> str:
    ...
    # If we didn't catch a specific dept, but it's a generic dept question,
    # just return total employees.
...
def answer_hr_attrition_by_agegroup(query: str) -> str:
    ...
    # Get age group with max attrition
...
if __name__ == "__main__":
    print("🚀 Launching Assistant (Sales + HR, deterministic + RAG)...")
    gr.Interface(
        fn=rag_query_ui,
        inputs="text",
        outputs="text",
